...
public class PlayerMovement : MonoBehaviour
{
   ...
   // [SerializeField] private float speedRetentionTime = 1.0f;   // How long speed is retained when changing direction
    ...
    [SerializeField] public AudioClip speedBoostSound;  // Add this line
...
[SerializeField] public bool enableDoubleJump = true;  // Enable/disable double jump feature
[SerializeField] public float doubleJumpForce = 10f;   // Force of the second jump (usually lower than first jump)
[SerializeField] public ParticleSystem doubleJumpParticles; // Optional particles for double jump
[SerializeField] public AudioClip doubleJumpSound;     // Optional sound for double jump


[Header("Footstep Settings")]
[SerializeField] private AudioClip[] footstepSounds;    // Array of different footstep sounds
[SerializeField] private float footstepInterval = 0.3f; // Time between footsteps
[SerializeField] private float minPitchVariation = 0.9f; // Minimum pitch variation
[SerializeField] private float maxPitchVariation = 1.1f; // Maximum pitch variation
[SerializeField] private float minVolumeThreshold = 0.5f; // Min speed before footsteps play
[SerializeField] private bool speedAffectsFootsteps = true; // Whether speed affects footstep rate
[SerializeField] private LayerMask footstepSurfaceLayers; // Different surfaces for different sounds


private float footstepTimer = 0f;
private int lastFootstepIndex = -1; // To avoid playing the same sound twice

// Jump tracking
private int jumpCount = 0;  // Track how many jumps performed
private int maxJumps = 2;   // Maximum number of jumps allowed (2 for double jump)
...
if (jumpInput) {
    ...
    // Mark jump as released when button is released
    if (!jumpInputHeld) {
...
private void Jump()
{
    // Track which jump this is
    jumpCount++;
    
    // Apply appropriate jump force
    float jumpPower = (jumpCount == 1) ? jumpForce : doubleJumpForce;
    ...
    // Play appropriate particles
    if (jumpCount == 1 && jumpParticles != null)
    {
        jumpParticles.Play();
    }
    else if (jumpCount > 1 && doubleJumpParticles != null)
    {
    ...
    if (audioSource != null)
    {
        AudioClip soundToPlay = (jumpCount == 1) ? jumpSound : doubleJumpSound;
        if (soundToPlay != null)
        {
...
{
    // Only play footsteps while grounded and moving faster than threshold
    if (isGrounded && Mathf.Abs(rb.linearVelocity.x) > minVolumeThreshold)
    {
        // Calculate interval based on speed if enabled
        float currentInterval = footstepInterval;
        if (speedAffectsFootsteps)
        {
            // Faster movement = faster footsteps (inversely proportional)
            float speedFactor = Mathf.InverseLerp(initialMoveSpeed, maxMoveSpeed, Mathf.Abs(rb.linearVelocity.x));
...
private void PlayFootstepSound()
{
    ...
    // Check surface type below player (optional)
    RaycastHit2D hit = Physics2D.Raycast(transform.position, Vector2.down, 1.5f, footstepSurfaceLayers);
    
    // Select sound based on surface (simple example)
    AudioClip soundToPlay;
    
    if (footstepSounds.Length == 1)
    {
        // If only one sound, use it
        soundToPlay = footstepSounds[0];
    }
    else
    {
        // Choose random sound that's not the same as last one
        int index;
        do
        {
        ...
        soundToPlay = footstepSounds[index];
    }
    
    // Calculate volume based on speed
    float speedRatio = Mathf.InverseLerp(minVolumeThreshold, maxSpeed, Mathf.Abs(rb.linearVelocity.x));
    float volume = Mathf.Lerp(0.2f, 1.0f, speedRatio);
...
{
    return enableDoubleJump && jumpCount == 1 && !isGrounded;
}
...
{
    return enableDoubleJump ? maxJumps : 1;
}
}